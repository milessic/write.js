...
def test_json_to_dict():
    ...
    assert json_to_dict("""{"chrząśżść":true,"isPies":1,"kamehameha":null}""") == {
        "chrząśżść": True,
        "isPies": 1,
        "kamehameha": None,
    }
    assert json_to_dict("""{"items":[[1,2,3],\n[2,3,4,null]]}""") == {
        "items": [[1, 2, 3], [2, 3, 4, None]]
    }
    assert json_to_dict(Model(login="hello!", number=123)) == {
        "login": "hello!",
        "number": 123,
    }
...
def test_generate_jwt_access_token(controller):
    ...
    assert (
        expected_expires - 10
        <= decode_token(token, controller).get("exp")
        <= expected_expires + 10
    )
