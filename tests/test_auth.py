...
def test_register_with_existing_username():
    response = client.post(
        auth_prefix + "register",
        json={"username": username, "password": valid_password, "email": "q" + email},
    )
    assert response.status_code == 400
...
def test_register_with_existing_email():
    response = client.post(
        auth_prefix + "register",
        json={"username": "q" + username, "password": valid_password, "email": email},
    )
    assert response.status_code == 400
...
def test_register_with_too_short_password():
    response = client.post(
        auth_prefix + "register",
        json={"username": "qw" + username, "password": "abc1!", "email": "qw" + email},
    )
    assert response.status_code == 400
...
def test_email_with_username_success():
    response = client.post(
        auth_prefix + "register",
        json={"username": username2, "password": valid_password, "email": email2},
    )
    assert response.status_code == 200
    assert response.json() == {"message": "registered"}
    response = client.post(
        auth_prefix + "token",
        data={"username": email2, "password": valid_password},
        headers={"Content-Type": "application/x-www-form-urlencoded"},
    )
    resp_json = json.loads(response.text)
    assert "access_token" in resp_json
...
def test_tokens_generated_too_quickly():
    response = client.post(
        auth_prefix + "register",
        json={"username": username3, "password": valid_password, "email": email3},
    )
    assert response.status_code == 200
    assert response.json() == {"message": "registered"}
    result = False
    for i in range(3):
        response = client.post(
            auth_prefix + "token",
            data={"username": email3, "password": valid_password},
            headers={"Content-Type": "application/x-www-form-urlencoded"},
        )
        resp_json = json.loads(response.text)
        if i == 0:
        ...
        assert (
            ...
            == "Access Token already exists! You may be generating it too fast!"
        )
        result = True
        break
    assert result
...
def test_check_endpoint_with_auth_via_cookie():
    ...
    # check /auth/me - 200
    resp_auth_me = client.get(
        auth_prefix + "me", cookies={"access_token": access_token}
    )
...
def test_reset_password_new_password_is_too_short():
    # reset password
    resp_reset_password = client.post(
        auth_prefix + "user/password/update",
        json={"old_password": valid_password, "new_password": "abcd"},
        headers={"Content-Type": "application/json"},
        cookies={"access_token": valid_token},
    )

    assert resp_reset_password.status_code == 400
    assert json.loads(json.loads(resp_reset_password.text).get("detail"))[
        0
    ] == c.locales.get_with_request("txt_errors_password_too_short", None).format(
        c.PASSWORD_MIN_LEN
    )
...
def test_reset_password_old_password_is_not_correct():
    ...
    resp_reset_password = client.post(
        auth_prefix + "user/password/update",
        json={"old_password": "abcdef", "new_password": "abcddsadasd"},
        headers={"Content-Type": "application/json"},
        cookies={"access_token": valid_token},
    )
    ...
    assert (
        json.loads(resp_reset_password.text)
        .get("detail")
        .startswith("Cannot change password!")
    )
...
def test_reset_password_invalid_token():
    resp_reset_password = client.post(
        auth_prefix + "user/password/update",
        json={"old_password": valid_password, "new_password": "abcd123123"},
        headers={"Content-Type": "application/json"},
        cookies={"access_token": invalid_token},
    )
    assert resp_reset_password.status_code == 401
    assert (
        json.loads(resp_reset_password.text).get("detail") == "Invalid or expired token"
    )
...
def test_reset_password_200():
    ...
    assert json.loads(resp_reset_password.text).get(
        "msg"
    ) == controller.locales.get_with_request("txt_password_changed", None)
...
def test_user_cannot_login_after_n_failed_attempts():
    ...
    resp_create_user = register_user(
        username=username, password=valid_password, email=f"{username}@text.com"
    )
...
def test_delete_user_is_not_deleted_if_wrong_password_is_provided():
    ...
    username = generate_username()
    resp_create_user = register_user(
        username=username, password=valid_password, email=f"{username}@test.com"
    )
    assert resp_create_user.status_code == 200
    ...
    delete_resp = client.post(
        auth_prefix + "delete",
        json={"old_password": valid_password + "abc", "are_you_sure": True},
        headers={"Content-Type": "application/json"},
        cookies={"access_token": valid_token},
    )
    ...
    assert (
        delete_resp.text
        == "{"
        + f'''"detail":"{c.locales.get_with_request("txt_errors_user_delete_wrong_password", None)}"'''
        + "}"
    )
...
def test_delete_user_is_not_deleted_if_user_is_not_sure():
    ...
    delete_resp = client.post(
        auth_prefix + "delete",
        json={"old_password": valid_password, "are_you_sure": False},
        headers={"Content-Type": "application/json"},
        cookies={"access_token": valid_token},
    )
    assert delete_resp.status_code == 400
    assert (
        delete_resp.text
        == "{"
        + f'''"detail":"{c.locales.get_with_request("txt_errors_user_delete_user_is_not_sure", None)}"'''
        + "}"
    )
...
def test_delete_user_401_if_token_not_provided():
    ...
    assert (
        delete_resp.text
        == "{"
        + f'''"detail":"{c.locales.get_with_request("txt_error_jwt_token_not_provided", None)}"'''
        + "}"
    )
...
def test_delete_user_is_deleted():
    ...
    delete_resp = client.post(
        auth_prefix + "delete",
        json={"old_password": valid_password, "are_you_sure": True},
        headers={"Content-Type": "application/json"},
    ...
    assert (
        token_resp_after_delete.text
        == "{"
        + f'''"detail":"{c.locales.get_with_request("txt_error_login_or_email_doesnt_exist", None)}"'''
        + "}"
    )
