...
def generate_username_response(errors: str, request: Request):
    if not (errors):
        raise ValueError(
            c.locales.get_with_request("txt_errors_unexpected_errors", request)
        )
    errors_list = []
            ...
            case "a":
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_username_is_already_taken", request
                    )
                )
            case "s":
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_username_invalid_characters", request
                    )
                )
            case _:
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_username_some_other_problem".format(errors), request
                    )
                )
        return errors_list
    ...
    for letter in str(errors):
        match letter:
            case "q":
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_password_too_short", request
                    ).format(c.PASSWORD_MIN_LEN)
                )
            case "w":
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_password_too_long", request
                    ).format(c.PASSWORD_MAX_LEN)
                )
            case _:
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_password_some_other_problem".format(errors), request
                    )
                )
...
def generate_email_response(errors: str, request: Request):
    ...
    for letter in errors:
        match letter:
            case "z":
                errors_list.append(
                    c.locales.get_with_request("txt_errors_email_taken", request)
                )
            case "x":
                errors_list.append(
                    c.locales.get_with_request("txt_errors_email_invalid", request)
                )
            case _:
                errors_list.append(
                    c.locales.get_with_request(
                        "txt_errors_email_some_other_problem".format(errors), request
                    )
                )
        return errors_list
