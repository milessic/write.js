...
def generate_access_token(sub: str, request: Request, controller: Controller = c):
    expire = datetime.now(UTC) + timedelta(
        minutes=controller.ACCESS_TOKEN_EXPIRES_MINUTES
    )
        ...
        raise HTTPException(
            500,
            {
                "msg": c.locales.get_with_request(
                    "txt_error_jwt_user_not_found", request
                ).format(sub)
            },
        )
    controller.db.create_access_token_record(
        token, user_id, get_epoch_now() + 60 * controller.ACCESS_TOKEN_EXPIRES_MINUTES
    )
    return token
...
def generate_refresh_token(sub: str, request, controller: Controller = c):
    expire = datetime.now(UTC) + timedelta(
        minutes=controller.REFRESH_TOKEN_EXPIRES_MINUTES
    )
    jwt_data = {"sub": sub, "exp": expire}
    token = jwt.encode(jwt_data, controller.SECRET_KEY)
    user_id = controller.db.get_user_id_from_username(sub)
    ...
    controller.db.create_refresh_token_record(
        token, user_id, get_epoch_now() + 60 * controller.REFRESH_TOKEN_EXPIRES_MINUTES
    )
    return token
...
async def get_access_token(
    ...
    if not token:
        raise HTTPException(
            status_code=401,
            detail=c.locales.get_with_request(
                "txt_error_jwt_token_not_provided", request
            ),
        )
...
def verify_access_token(
    token: str | None, request: Request, controller: Controller = c
):
    if token is None:
        raise HTTPException(
            status_code=401,
            detail=controller.locales.get_with_request(
                "txt_error_jwt_invalid_or_expired_token", request
            ),
        )
...
def verify_jwt_token(
    token: str, scenario: str, request: Request, controller: Controller = c
):
    try:
        ...
        if scenario.startswith(
            "access"
        ) and not c.db.check_if_access_token_is_active_for_user(
            token, user_id=user_id, expires=get_epoch_now()
        ):
            raise jwt.exceptions.ExpiredSignatureError()
        elif scenario.startswith("refresh") and not (
            x := c.db.check_if_refresh_token_is_active_for_user(
                token, user_id=user_id, expires=get_epoch_now()
            )
        ):
            raise jwt.exceptions.ExpiredSignatureError()
...
def generate_random_password() -> str:
    return "".join(
        SystemRandom().choice(ascii_letters + ascii_uppercase + ascii_lowercase)
        for _ in range(10)
    )
...
def check_if_user_can_login(user_id: int, controller: Controller = c) -> None:
    ...
    if (
        controller.db.get_failed_login_attempts(user_id, expires)
