...
class MailClient:
    def __init__(
        ...
        config,
        silent: bool = False,
        initialize_smtp: bool = True,
        initialize_imap: bool = True,
        ...
        if self.initialize_credentials:
            ...
            if self.credentials.password is None:
                raise AssertionError(
                    f"{self.style.err}Password was None! If running from cron, please reffer to README!{self.style.endc}"
                )
            self.config = config
        if initialize_smtp:
            # setup smtp
            try:
                ...
                raise AssertionError(
                    f"{self.style.err}Error when connecting to SMTP server! {type(e).__name__}: {e}{self.style.endc}\n{self.style.warn}{add_e}{self.style.endc}"
                )
        # setup IMAP
        if self.initialize_imap:
            try:
                ...
                self.i.login(
                    user=self.credentials.login, password=self.credentials.password
                )
            except Exception as e:
                print(
                    f"{e}Error connecting to IMAP server! {type(e).__name__}: {e}{self.style.endc}"
                )
    ...
    def add_to_favorites(self):
        ...
        confirm = (
            True
            if input(f"Mail: '{mail}', alias: '{alias}'\nDo you confirm? y/n ")
            .lower()
            .startswith("y")
            else False
        )
        if confirm:
            print(
                f"{self.style.deep_green}{alias} added to Favorites.{self.style.endc}"
            )
    ...
    def remove_from_favorites(self):
        ...
        if alias not in self.favorites:
            print(
                f"{self.style.deep_green}There are no '{alias}' in FAVORITES!{self.style.endc}"
            )
            self.press_enter()
            return
        confirm = (
            True
            if input(
                f"Mail: '{self.favorites[alias]}', alias: '{alias}'\nDo you confirm deletion? y/n "
            )
            .lower()
            .startswith("y")
            else False
    ...
    def _setup_message(
        self, to, subject, content, content_type="plain"
    ) -> MIMEMultipart:
        msg = MIMEMultipart()
        msg["Subject"] = subject
        msg["To"] = to
    ...
    def send_mail(self):
        try:
            ...
            msg = MIMEMultipart()
            msg["Subject"] = input(
                f"{self.style.deep_green}SUBJECT >>>{self.style.endc} "
            )
            recipients_raw = input(
                f"{self.style.deep_green}RECIPIENTS >>>{self.style.endc} "
            ).split(",")
            recipients = ",".join(
                [
                    self.favorites[r] if r in self.favorites else r
            ...
            msg.attach(
                MIMEText(
                    self.collect_multiline(f"{self.style.green}TYPE BODY:"), "plain"
                )
            )
    ...
    def show_inbox(self):
        clear()
        print(
            f"{self.style.green}=== INBOX {self.style.endc}{self.style.b if self.message_counts['unread'] else self.style.endc}{self.message_counts['unread']}{self.style.endc} unread of {self.message_counts['all']}"
        )
        self._fetch_emails_imap()
        print(tabulate([m for m in self.messages_short], headers="keys"))
        inbox_input = input(
            self.style.green
            + "[0] Read {Num} - [1] Reply {Num} - [2] Reload [3] Delete {num} - [9] Exit\n"
            + self.style.b
            + ">>> "
            + self.style.endc
        )
        match inbox_input.split(" ")[0].upper():
            case "0" | "READ":
                ...
                try:
                    ...
                    input(
                        f"{err} There is no message with given num '{num}'!\nPress ENTER to continue{self.style.endc} "
                    )
                finally:
                    self.show_inbox()
            case "1" | "REPLY":
    ...
    def open_message(self, num):
        ...
        msg_input = input(
            self.style.green
            + "[0] Reply [1] Forward [2] Delete [3] Download {path} [9] Back\n"
            + self.style.b
            + ">>> "
            + self.style.endc
        )
    ...
    def _parse_mail(self, mail: dict) -> str:
        if mail["Content-Type"].startswith("text/plain"):
            ...
            mail_content = (
                self.style.warn
                + f"< {re.sub(pattern,'',mail['Content-Type'])} not supported yet! Only text/plain is supported! >"
                + self.style.endc
            )
...
----------------------------------{nl + self.style.green +"[4] MORE" + self.style.endc if mail_content_more is not None else ""}"""
        ...
        for i in range(1, int(messages[0]) + 1):
            ...
            for response in msg:
                if isinstance(response, tuple):
                    ...
                    msg_count["all"] += 1
                    msg_subject = (
                        f"{self.style.endc if msg_seen else self.style.b}"
                        + msg["Subject"]
                        + self.style.endc
                    )
                    msg_from = msg["From"]
    ...
    def not_implemented(self, msg: str = ""):
        input(
            self.style.warn
            + "Not implemented! press ENTER to continue"
            + msg
            + self.style.endc
        )
